...
role belongs to `theaamghar-web`/`theaamghar-admin` (`E:\Claude\theaamghar-web`,
`E:\Claude\theaamghar-admin`, Supabase project `eznxsosvsgkhexbjoolh`), which
...
see `theaamghar-admin/CLAUDE.md`'s "This is now the real multi-vendor
...
- Vendor applications (`/apply` → `vendor_applications` → admin approval →
  ...
  onboarding funnel feeding vendors *into* `theaamghar-web`'s engine, not a
  storefront itself.
...
- The public homepage's "Real stores, already selling" directory —
  correctly links out to each vendor's real `theaamghar-web`-powered
...
`theaamghar-web`/`theaamghar-admin` Vercel projects, NOT this one — assigned
