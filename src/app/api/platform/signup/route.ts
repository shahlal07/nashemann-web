...
export async function POST(request: Request) {
  try {
    ...
    if (existingUsers.users.some((u) => u.email?.toLowerCase() === email)) {
      return NextResponse.json({ error: "An account with this email already exists. Use the matching login role." }, { status: 409 });
    }
    ...
    if (existingVendorAdmin) {
      return NextResponse.json({ error: "This email is already a storefront vendor-admin account. Use Revenue View login." }, { status: 409 });
    }
